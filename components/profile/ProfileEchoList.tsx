 ...
 * - [KEEP] Partage accessible même sans login (Web Share / clipboard)
 *
 ...
 * - [KEEP] CommentsModal lecture toujours possible ; ajout seulement si connecté (canPost)
 *
 * KEEP / SAFE
 ...
 * - Merge ids présents dans echoes dans le state existant
 * - Jamais de décrément, jamais d’écrasement à la baisse
 */
function mergeCommentsCounts(prev: Record<string, number>, echoes: PublicEcho[]): Record<string, number> {
  ...
  for (const e of echoes ?? []) {
    ...
    const cur = next[e.id];
    if (typeof cur !== 'number') next[e.id] = fromProps;
    else next[e.id] = Math.max(cur, fromProps);
  }
  return next;
}
...
function EchoCard({
  ...
  const location = useMemo(
    () => [echo.city, echo.country].filter(Boolean).join(', '),
  ...
  const emo = useMemo(
    () => emotionLabel((echo as unknown as { emotion?: string | null }).emotion ?? null),
    [echo]
  );
  ...
  // Lecture commentaires doit rester possible sans login
  const handleComment = () => {
  ...
  return (
    <>
        ...
        <div className="border-t border-slate-100 px-5 py-3">
          <div className="flex items-center gap-6 text-sm text-slate-600">
            ...
            <button
              ...
              title={currentUserId ? 'Voir / répondre' : 'Voir les commentaires (connexion requise pour répondre)'}
            >
      ...
      {shareOpen ? (
        <ShareModal echoId={echo.id} onClose={() => setShareOpen(false)} userId={currentUserId ?? null} />
      ) : null}
