 ...
 * Objet        : Typing indicator "... Alice écrit" — LOT 2.6 + fixes
 * -----------------------------------------------------------------------------
 * CHANGELOG
 * -----------------------------------------------------------------------------
 * 1.1.0 (2026-01-25)
 * - [FIX] Animation points AVANT texte (... Alice écrit)
 * - [FIX] Position sticky au-dessus composer (toujours visible)
 * - [FIX] Padding réduit pour gagner espace
 * =============================================================================
 */
...
export default function TypingIndicator({ typingUsers, currentUserId, variant = 'page' }: Props) {
  ...
  return (
    <div
      className={`flex items-center gap-2 border-t border-slate-100 bg-white/95 text-slate-600 backdrop-blur ${
        isDock ? 'px-3 py-1.5 text-xs' : 'px-5 py-2 text-sm'
      }`}
    >
      {/* ✅ FIX: Points AVANT le texte */}
