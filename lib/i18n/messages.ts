 ...
 * - [FIX] Fusionne ABOUT_MESSAGES dans MESSAGES (les clés about.* deviennent résolubles).
 * - [FIX] Supprime l'import "unused" (ABOUT_MESSAGES est réellement utilisé).
 * - [FIX] Évite les casts dangereux Record<I18nKey,string> qui déclenchaient TS2352.
 * - [SAFE] Les clés core existantes restent strictement typées (anti-régression).
 * =============================================================================
 */
...
/**
 * =============================================================================
 * KEYS — GLOBAL (source de vérité) — CORE UNIQUEMENT (anti-régression)
 * =============================================================================
 */
export type I18nKey =
  // Common
 ...
 * DICT
 * -----------------------------------------------------------------------------
 * Dict “final” : accepte les clés core + toutes les clés additionnelles (ex: about.*).
 * BUT : conserver le typage strict sur BASE_MESSAGES, tout en fusionnant des modules.
 * =============================================================================
 */
...
/**
 * =============================================================================
 * BASE MESSAGES (STRICT) — toutes les clés I18nKey doivent être présentes
 * =============================================================================
 */
const BASE_MESSAGES: Record<AppLang, Record<I18nKey, string>> = {
  en: {
    // Common
 ...
 * MESSAGES (FINAL)
 * -----------------------------------------------------------------------------
 * On fusionne BASE_MESSAGES (strict) + ABOUT_MESSAGES (module /about).
 * Résultat : t('about.xxx') peut fonctionner dès que tes composants l’utilisent.
 * =============================================================================
 */
export const MESSAGES: Record<AppLang, Dict> = (Object.keys(
  BASE_MESSAGES
) as AppLang[]).reduce((acc, lang) => {
  const aboutPart = (ABOUT_MESSAGES?.[lang] ?? {}) as Dict;
  acc[lang] = { ...BASE_MESSAGES[lang], ...aboutPart };
  return acc;
}, {} as Record<AppLang, Dict>);
